...
| なぜ 2DoF 以上の冗長性があるか | 後述（§3）。両腕＋胴体 13DoF に対し主タスクは 5 次元（前後フリー） → **冗長 8DoF** |
...
| **球保持タスク** | **台車 (base) 座標** | 両腕＋胴体 (13DoF) のヌル空間制御 | 球を台車正面・一定高さに保ち続ける |

→ **課題＝「球を台車の真正面・一定の高さに保つ（前後は自由）」。PAD による表現はこの球保持タスクのヌル空間を使う。**
台車が世界座標でどこへ動こうと、台車座標から見た球は正面・一定高さに留まる。これにより移動と上半身の表現は分離される。

### 主タスク（球保持タスク, primary task）
**台車 (base) 座標系**で、両手で持つ球に対し次の **5 次元**だけを拘束する（緩和済み）。
`m = (p_Lhand + p_Rhand)/2`（両手中点）、`d = p_Lhand − p_Rhand`（相対ベクトル）として:

```
x = [ d(3) , m_y , m_z ] ∈ R^5      （台車座標系で計測）
  把持維持:   d = const      … 両手の相対位置を固定し球を落とさない (3)
  真正面:     m_y = 0        … 球を左右中心（矢状面上）に (1)
  一定の高さ: m_z = z0       … 球の高さを固定 (1)
  前後 m_x:   自由           … 球の前後位置は拘束しない（めり込まない限り）
```

- **前後 (`m_x`) を解放**したのが今回の緩和。腕が届きにくい・拘束がきつい問題を避け、かつ前後並進が**新たな表現自由度**になる（球を前に押し出す／引き寄せる動き）。
- 手先の**姿勢（向き）は拘束しない**（球は開いた手のひらで下から支える前提）。
- 計測フレームは「台車 (base)」。腕より下の胴体関節 (HipRoll/HipPitch/KneePitch) も球-対-台車を動かすため**胴体も制御チェーンに入り**、前傾などの胴体表現を「腕が補償して球を正面・一定高さに保つ」全身ヌル空間運動として実現できる。
...
### 冗長性 DoF
```
冗長 DoF = dim(q) - rank(J) = 13 - 5 = 8 DoF   ← 課題要件「2DoF 以上」を十分満たす ✓
```
- 主タスクは 5 次元（把持3＋左右中心1＋高さ1）。前後を解放したぶん冗長性が 7→**8 DoF** に増えた。
- 胴体 3DoF は丸ごと冗長で、傾けても腕が補償して球を正面・一定高さに保てる。
- → 主タスク（球を正面・一定高さ）を乱さずに、肘・前腕・胴体姿勢＋球の前後位置を変えられる。これがヌル空間表現の土台。
...
q̇ = J⁺ ẋ_d  +  (I − J⁺J) q̇₀
```
- `J = ∂x/∂q ∈ R^{5×n}`: 緩和タスク `x = [d, m_y, m_z]` のヤコビアン。左右手先位置の積み上げ並進ヤコビアン（`calculateJacobian`）から、`d = p_L − p_R` の3行と中点 `m` の y,z 行を作って構成（前後 `m_x` の行は落とす）。
...
- [x] **M2 運動学**（完了, `kinematics.py`）: `calculateJacobian` ラッパ (`hand_jacobian`: 並進＋角速度)、腕限定タスク優先 DLS IK (`solve_arm_ik`)、および**緩和保持タスクのヤコビアン** `holding_task_jacobian` を実装。左右手先の並進ヤコビアン (CoM 基準 = `calculateJacobian` の localPosition と一致) を台車座標へ回し、`d=p_L−p_R` の3行＋中点 `m` の y,z 行を積み上げ `J∈R^{5×13}` を構成。`scripts/verify_jacobian.py` の中心差分で数値検証 → `max|J−Jfd| ≈ 9e-12` で **PASS**、`rank(J)=5` → **冗長 8DoF** を確認。
  - 注意点: ① `calculateJacobian(localPosition=[0,0,0])` はリンク**重心(CoM, `getLinkState[0]`)**のヤコビアンなので、タスク値も CoM 基準で測る (`link_com_position`)。② 台車座標変換 `base_frame_point` は numpy の `R^T(p−base)` で実装（pybullet の invert/multiplyTransforms 往復は ~1e-8 の量子化誤差を生み、中心差分 `÷2eps` で 1e-3 に増幅され誤検出の原因になる）。
- [x] **M3 コントローラ核**（完了, `controller.py`）: `NullSpaceController` が `q̇ = J⁺ẋ_d + (I−J⁺J)q̇₀`（DLS 擬似逆 `Jᵀ(JJᵀ+λ²I)⁻¹`, λ=0.05, K_task=4）を速度レベルで解き、積分した関節角を `resetJointState`＋`setAngles` で書き戻すキネマティック運用。`scripts/demo_nullspace.py` で検証:
  - **(A) baseline** `q̇₀=0`: 拘束 5 次元の誤差 `~1e-5 m` で球を静的保持 ✓。
  - **(B) 冗長性** `q̇₀` を HipPitch/ElbowRoll に印加→ヌル空間 `N` 射影: 胴体が前傾 (HipPitch +0.97rad)・膝屈曲、肩肘が自動補償。拘束 (把持 d / 真正面 m_y / 高さ m_z) は `Δ≲4mm` で不変、自由次元 m_x(前後) のみ動く。`outputs/m3_before.png`→`m3_after.png` で「姿勢だけ大きく変わり球は正面・一定高さを保持」を可視化 ✓。null-space dim = 8 を実機確認。
- [x] **M4 PAD マッピング**（完了, `pad.py`）: `PADExpression` が `q*_PAD = q_neutral + P·δ_P + A·δ_A + D·δ_D` と律動成分 `amp(A)·ω(A)·cos(ωt)·d_dir` から二次目標 `q̇₀` を生成（目標姿勢は t_ramp 秒でイーズイン）。頭部 (HeadPitch) は手先に無関係なので直接駆動。`scripts/demo_pad.py` で要求 5 PAD を各 2.5s 実行し検証:
  - 全 PAD で拘束タスク誤差 **< 4mm**（球を落とさない）。`k_task=10` でタイトに保持。
  - 見分け: `[0,0,0]` 中立直立 / `[1,1,1]` 直立・頭上・球を前へ差し出す / `[-1,-1,-1]` 前傾・頭下・球を抱え込み静止 / `[-1,1,-1]` 抱え込み＋頭下＋**律動（震え）** / `[-1,1,1]` 直立・球前・**律動** で威圧。joy/anger・sad/fear の各ペアは主に**覚醒の律動有無**で動的に差別化。
  - 設計上の要点: 深い前傾は腕の保持リーチと拘束 (m_z 一定高さ) が競合 → タスク優先で傾きが自動制限される（球が沈まない）。縮こまり側は肘を曲げてタックさせ保持リーチを確保。前後 `m_x` 自由が「差し出す↔抱え込む」のコントラストを生む。`outputs/pad_*_{persp,front}.png` 参照。
- [x] **M5 パス追従**（完了, `path.py`）: 平面パス生成 (`LinePath`/`SCurvePath`/`CirclePath`、`pose(t)→(x,y,yaw)`) と台車追従 (`apply_base_pose` = `resetBasePositionAndOrientation`)。キネマティック運用なので qibullet の `moveTo`(物理ステップ依存)は使わず base 姿勢を直接設定。`scripts/demo_path.py` で S 字パス(8s, 並進＋yaw±60°旋回)＋PAD表現を統合検証:
  - 移動中も拘束タスク誤差 **< 1cm**、球の台車座標 y(真正面) ズレ < 5mm・z(一定高さ) ズレ < 2mm → 球は台車正面・一定高さを保持 **PASS**。
  - **移動と表現の分離**を実機確認: 保持タスクが台車座標で定義されているため、台車を剛体的に動かす(並進・旋回)だけでロボット全体が一緒に動き保持タスク誤差は出ない。上半身の PAD 表現(ヌル空間)はこれと独立に並走。`outputs/m5_path_0..5.png` で「曲がっても球は正面・一定高さ」を可視化。
- [x] **M6 録画**（完了, `record.py`）: パス追従＋PAD表現を回しつつ `getCameraImage` でフレーム取得、PIL で「ロボット名／タスク／PAD値＋感情ラベル／冗長性」と進捗バーを焼き込み `imageio`(libx264) で mp4 出力。カメラは台車追従でロボットを中央に大きく映す。`scripts/record_demo.py` で要求 5 PAD を同一パス(S字8s)・同一カメラ・30fps で録画。
  - 出力: `outputs/pad_{000_baseline,111_joy,nnn_sad,n1n_fear,n11_anger}.mp4`（各 ~1MB, 241フレーム）。全クリップで移動中タスク誤差 **< 1.1cm**（球保持）。
  - 表情差: baseline=中立直立 / joy=頭上・球を前へ / sad=頭下・前傾・抱え込み / fear=縮こまり＋律動(震え) / anger=直立＋律動(威圧)。sad↔fear・baseline↔anger は覚醒の律動有無で動的に差別化（PAD 設計どおり A は時間チャンネル）。
...
- 冒頭にテキストで「Robot: Pepper / Task: carry a 0.25m sphere along a planar path (sphere kept in front at constant height; fore-aft free) / Redundancy: 13 joints − 5D task = 8 DoF」を表示。

...
| どのタスクか | **直径 50cm の球体を胸の前で両手で抱えたまま、平面上のパスを辿る** |
...
- 球体は PyBullet のプリミティブ（半径 0.25m の sphere）で生成し、両手の中点に **固定拘束 (`createConstraint`)** で取り付けるか、毎ステップ両手中点へキネマティックに配置する（動力学の落下を避ける）。
...
- 「球と台車の相対位置を固定」＝両手先を台車基準の保持位置（胸前、左右に半径ぶん離す ≒ 50cm 間隔）に保つことが主タスク。
...
├── src/pepper_pad/
│   ├── __init__.py
│   ├── sim.py               # Pepper・地面・50cm 球のロード／シーン構築
...
- [x] **M0 環境構築**（完了）: uv プロジェクト作成（Python 3.11）、依存導入（pybullet/numpy/imageio/qibullet）、`.gitignore`/README 整備、qibullet メッシュ展開、DIRECT モードでスモークテスト成功（`scripts/smoke_test.py`）。
  ...
  - スモーク結果: 既定姿勢で手先間隔 ≈ 0.33m（`l_hand`/`r_hand`）。**直径 0.5m の球を抱えるには M1 で間隔 ≈ 0.5m の保持姿勢に調整が必要**。
- [ ] **M1 シーン構築** (`sim.py`): Pepper＋地面＋直径 50cm 球を生成。両腕を左右対称の保持姿勢へ。球を両手中点に拘束。視覚的に「抱えている」状態を確認。
- [ ] **M2 運動学** (`kinematics.py`): `calculateJacobian` で左右手先の並進ヤコビアンを構成し `J∈R^{6×n}` を組み立て。有限差分で FK／ヤコビアンを数値検証。
...
- 冒頭にテキストで「Robot: Pepper / Task: carry a 0.5m sphere along a planar path (sphere fixed relative to the base) / Redundancy: 13 joints − 6D task = 7 DoF」を表示。
